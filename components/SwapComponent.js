...
import{
  ...
  const SwapComponent = () => {
    ...
    useEffect(() => {
      ...
      setDestTokenComp(<SwapField obj={destokenObj} ref={outputValueRef} />);

      if(outputValue?.length === 0) setInputValue("");

      //Resetting if isReversed value if its set
    ...
    async function performSwap(){
      ...
      else receipt = await swapEthToToken(srcToken, destToken, inputValue);
